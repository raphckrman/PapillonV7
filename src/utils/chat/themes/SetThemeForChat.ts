...
async function DownloadTheme (meta: ThemesMeta): Promise<boolean> {
  let f_theme = await fetch("https://raw.githubusercontent.com/raphckrman/chat-themes/main/" + meta.path + "/theme.json");
  ...
  for (let i = 0; i < to_download.length; i++) {
    let f = await fetch("https://raw.githubusercontent.com/raphckrman/chat-themes/refs/heads/main/" + meta.path + "/" + to_download[i]);
    let r = await f.blob();
